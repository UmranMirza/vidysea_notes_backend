...
from application.schemas.app.user_schemas import UserRegister, Userlogin
...
def register_user(request:Request, data: UserRegister, db:Session=activeSession):
    ...
    user = UserService().register_user(db=db,role=data.role, name=data.name, email=data.email, password=data.password,phone=data.phone)
    user_serialized = {
          "id": user.id,
          "name": user.name,
          "email": user.email,
    ...
    token = AuthService().generate_access_token(user_type=user.role, user=user_serialized)
    if user:
...
def login_user(request: Request, data: Userlogin, db: Session = activeSession):
    # Check if email already registered
    user = UserService().get_user_by_email(db=db, email=data.email)
    if user is None:
    ...
    if not auth_service.verify(data.password, user.hashed_password):
        return APIError(
